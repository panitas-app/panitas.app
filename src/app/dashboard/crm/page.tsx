  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-heading font-extrabold text-foreground">CRM</h1>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <Card className="rounded-xl border border-border">
          <CardContent className="p-4">
            ...
            <p className="text-xl font-bold text-foreground mt-1">{stats.total || customers.length}</p>
          </CardContent>
        </Card>
        <Card className="rounded-xl border border-border">
          <CardContent className="p-4">
            <p className="text-[10px] font-semibold text-muted-foreground uppercase">Recurrentes</p>
            <p className="text-xl font-bold text-foreground mt-1">{stats.recurrentes || customers.filter((c) => c.totalOrders > 1).length}</p>
          </CardContent>
        </Card>
        <Card className="rounded-xl border border-border">
          <CardContent className="p-4">
            <p className="text-[10px] font-semibold text-muted-foreground uppercase">Seguimientos</p>
            <p className="text-xl font-bold text-foreground mt-1">{stats.seguimientos || 0}</p>
          </CardContent>
        </Card>
        <Card className="rounded-xl border border-border">
          <CardContent className="p-4">
            <p className="text-[10px] font-semibold text-muted-foreground uppercase">Inactivos</p>
            <p className="text-xl font-bold text-foreground mt-1">{stats.inactivos || customers.filter((c) => !c.lastPurchaseAt || new Date(c.lastPurchaseAt) < inactiveThreshold).length}</p>
          </CardContent>
        </Card>
      </div>
                ...
                <CardContent className="p-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <div className="text-right">
                      <p className="text-xs font-bold text-foreground">{c.totalOrders} pedidos</p>
