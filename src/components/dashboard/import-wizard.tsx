...
import { Upload, FileSpreadsheet, CheckCircle, AlertCircle, ChevronRight, ChevronLeft, Download } from "lucide-react"
...
export function ImportWizard({ storeId, categories }: Props) {
  ...
  const allRequiredMapped = REQUIRED_FIELDS.every((field) =>
    columns.some((col) => col.mappedField === field)
  )
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span className={step === "mapping" ? "font-bold text-foreground" : ""}>2. Mapear columnas</span>
        <ChevronRight className="size-4" />
      ...
      {step === "mapping" && (
        <Card>
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              {totalRows} filas detectadas. Asigna cada columna del archivo a un campo de producto.
            ...
            {!allRequiredMapped && (
              <div className="flex items-center gap-2 p-3 text-sm text-yellow-600 bg-yellow-50 border border-yellow-200 rounded-lg">
                <AlertCircle className="size-4 shrink-0" />
                Debes mapear al menos: Nombre y Precio (*)
              </div>
            )}
            ...
            <div className="flex justify-between pt-4">
              ...
              <Button onClick={handleImport} disabled={!allRequiredMapped || loading}>
                {loading ? (
                  <>
