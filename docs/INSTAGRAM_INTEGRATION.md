# Integración de Instagram (FASE 8B)

> Guía técnica de la integración de **Instagram Direct Messages (Meta Graph API
> Messaging API)** como proveedor real de la Communication Integration Layer
> (FASE 7C). Reutiliza el mismo base compartido que Messenger (`MetaMessagingProvider`)
...
| Pieza | Estado |
|---|---|
| Provider real `InstagramProvider` (Graph API v22.0) | Implementado |
...
| Feature gating Panitas Plus (`instagram_inbox`) | Implementado |
...
1. En el plan Panitas Negocios Plus, abre **Conversaciones** → botón **Instagram**.
2. Pega los datos de tu Meta App / cuenta de Instagram:
   - **Access token** (token de la Meta App con permisos de Instagram Messaging:
     `instagram_basic`, `instagram_manage_messages` y `pages_manage_metadata`).
   - **Account id (ig id)** — id numérico de la cuenta de Instagram con la que
     se chatea, ej. `17841405812345678`.
   - Opcionalmente: nombre de la cuenta, verify token y app secret del webhook.
...
```
GET  https://<tu-dominio>/api/webhooks/instagram
     ?hub.mode=subscribe&hub.verify_token=<token>&hub.challenge=<challenge>
POST https://<tu-dominio>/api/webhooks/instagram
```
...
- **POST** exige firma `x-hub-signature-256` (HMAC-SHA256 con el App Secret de
  ...
  registra `instagram.webhook.*` y se responde 200 (`status: "ignored"`) para
  cortar los reintentos de Meta.
- El negocio se resuelve por el account id (`entry[].id`, guardado en
...
MetaMessagingProvider (base abstracta) ── src/lib/communication/providers/meta-messaging-provider.ts
  ...
  └─ media.ts                (resolución de URL por mediaId + proxy + fallback)
      │
      ▼
Inbox 7A (conversaciones, mensajes, clientes) — sin lógica Instagram
```
...
| `externalRef` | account id (ig id) → resuelve el negocio en webhooks |
...
- Cada mutación emite `instagram.connection.*` y registra auditoría.

## 6. Eventos

Registrados en `src/lib/events/event-registry.ts`:

- `instagram.connection.created|connected|disconnected|error|revoked`
- `instagram.webhook.received|invalid_signature|unresolved`
- `instagram.message.incoming|sent|delivered|read|failed`
...
3. Si hay conexión activa → `InstagramProvider.sendMessage` (guarda el `mid`
   real como `externalId`); si no → mock (config-gated).
4. Los webhooks `delivery`/`read` actualizan el estado a delivered/read.

## 8. Media

`GET /api/inbox/channels/instagram/media/[mediaId]` localiza la URL del adjunto
...
- `instagram_inbox` (Panitas Negocios Plus) protege la UI y la API de conexión.
...
1. Crear la Meta App, añadir el producto Instagram y vincular una cuenta.
2. Configurar el webhook en Meta hacia `/api/webhooks/instagram`.
3. Conectar desde la UI y verificar salud real.
4. Probar envío/ingesta/media con cuenta de prueba de Instagram.
